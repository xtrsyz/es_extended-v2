...
-- Properties
self.Config = ESX.EvalFile(GetCurrentResourceName(),
                           'modules/accessories/data/config.lua',
                           {vector3 = vector3})['Config']

self.Init = function()

    self.RegisterControls()

    local translations = ESX.EvalFile(GetCurrentResourceName(),
                                      'modules/accessories/data/locales/' ..
                                          Config.Locale .. '.lua')['Translations']
    ...
    for k, v in pairs(self.Config.Zones) do
        for i = 1, #v.Pos, 1 do
            ...
            AddEventHandler('esx:interact:exit:' .. key,
                            function(data) self.CurrentAction = nil end)
    ...
    for k, v in pairs(self.Config.ShopsBlips) do
        if v.Pos ~= nil then
            for i = 1, #v.Pos, 1 do
                ...
                AddTextComponentString(_U('accessories:shop',
                                          _U('accessories:' .. string.lower(k))))
...
self.SetUnsetAccessory = function(accessory)
    ESX.TriggerServerCallback('esx_accessories:get',
                              function(hasAccessory, accessorySkin)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
            ...
            elements = {
                {label = _U('accessories:no'), value = 'no'}, {
                    label = _U('accessories:yes',
                               ESX.Math.GroupDigits(self.Config.Price)),
                    value = 'yes'
                }
            }
        }, function(data, menu)
            ...
            if data.current.value == 'yes' then
                ESX.TriggerServerCallback('esx_accessories:checkMoney',
                    ...
                    if hasEnoughMoney then
                        ...
                        TriggerEvent('skinchanger:getSkin', function(skin)
                            TriggerServerEvent('esx_accessories:save', skin,
                                               accessory)
                        end)
                    else
