...
--- @function xPlayer.getFromIdentifier
--- Get xPlayer from identifier
--- @param identifier string Player identifier
--- @return xPlayer
xPlayer.getFromIdentifier = function(identifier)
...
xPlayer.load = function(identifier, playerId, cb)
  ...
  MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', { ['@identifier'] = identifier }, function(result)
    ...
    Async.parallelLimit(tasks, 5, function(results)
      ...
      xPlayer:emit('esx:playerLoaded', xPlayer:serialize())
      xPlayer:emit('esx:createMissingPickups', ESX.Pickups)
      xPlayer:emit('esx:registerSuggestions', ESX.RegisteredCommands)
...
self.onPlayerJoined = function(playerId)
  ...
  if identifier then
		...
		if xPlayer.getFromIdentifier(identifier) then
